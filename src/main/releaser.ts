...
function getFilename() {
  ...
  switch (osPlat) {
    case 'win32': {
      return `heycart-cli_Windows_${arch}.zip`;
    }
    case 'darwin': {
      return `heycart-cli_Darwin_${arch}.tar.gz`;
    }
    default: {
      return `heycart-cli_Linux_${arch}.tar.gz`;
    }
  }
}
